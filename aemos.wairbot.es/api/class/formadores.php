...
class Formadores{
    ...
    public static $tabla = 'formadores';
    ...
    public static function crear($request){
        ...
        if($created){
            if($created['status'] == 'success'){
                ...
                return [
                    'status' => 'success',
                    'message' => 'Formador creado con éxito',
                    'data' => [
                        'id' => $dataId
                    ]
                ];
            ...
            return [
                'status' => 'error',
                'message' => 'Error al crear el formador',
                'error' => $created
            ];
        }
    }
    ...
    public static function editar($request){
        ...
        if($updated){
            return [
                'status' => 'success',
                'message' => 'Formador actualizado con éxito',
            ];

        }else{
            return [
                'status' => 'error',
                'message' => 'Error al actualizar el formador',
                'error' => $updated
            ];
        }
    }
    ...
    public static function eliminar($request){
        ...
        //obtener datos del formador
        $sql = "SELECT * FROM $tabla WHERE id = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param('i', $id);
        $stmt->execute();
        $formador = $stmt->get_result();
        $formador = $formador->fetch_assoc();

        //cambiar id a id_original
        $formador['id_original'] = $formador['id'];
        unset($formador['id']);

        //crear formador en la tabla formadores_baja
        $tablaBaja = 'formadores_baja';
        Tools::create($conn, $tablaBaja, $formador, []);

        $check = [
            'inscripciones' => 'id_formador',
            'mensajes_programados' => 'id_formador',
            'acciones_formativas' => 'id_formador',
        ];
        ...
        if($deleted){
            ...
            return [
                'status' => 'error',
                'message' => 'Error al eliminar el formador',
                'error' => $deleted
            ];
        }
    }
    ...
    public static function listar($request){
        ...
        return [
            'status' => 'success',
            'message' => 'Formadores listados con éxito',
            'data' => [
    ...
    public static function obtenerPorId($request){
        ...
        //si el formador no existe
        if(!$result){
            return [
                'status' => 'error',
                'message' => 'El formador no existe',
            ];
        }

        return [
            'status' => 'success',
            'message' => 'Formador obtenido con éxito',
            'data' => $result,
        ];
    }
