...
async function openVendidoAgente(id) {
    ...
    let form = formVendidoAgente(maquina);
...
function formVendidoAgente(maquina) {
    let data = maquina[0];
...
async function openParteAgente(id) {
    ...
    let cardsMaquina = cardsVendidosAgente([maquina[0]]);
    ...
    //abrir modal
    let titulo = "Parte";
    let contenido = form + gridMaquina + gridTareas + gridArchivos + gridImagenes + gridVideos;
...
async function guardarParteAgente(id = 0) {
    ...
    //volver a cargar el maquina
    openVendidoAgente(id_maquina);
}
