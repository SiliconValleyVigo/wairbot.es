...
async function openTecnico(id) {
    ...
    //modificar el input password para que solo se pueda escribir caracteres numericos sin cambiar el tipo de input
    let inputPassword = document.getElementById("password");
    console.log(inputPassword);

    //añadir oninput eliminarNoNumerico
    inputPassword.setAttribute("oninput", "eliminarNoNumerico(this)");
}
...
//función para eliminiar caracteres no numericos de un input
function eliminarNoNumerico(input){
    console.log("eliminarNoNumerico");
    let value = input.value;
    value = value.replace(/\D/g, '');
    input.value = value;

    //impedir que el valor sea mayor de 4 caracteres
    if(value.length > 4){
