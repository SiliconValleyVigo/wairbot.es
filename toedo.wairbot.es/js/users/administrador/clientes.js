async function getClientes(nPagina = 1) {
    ...
    //eliminar de notificaciones los repetidos en data
    let notificacionesFiltradas = notificaciones.filter(notificacion => {
        return !data.some(cliente => cliente.id === notificacion.id);
    });

    //añadir notificacionesFiltradas a data pero en primeras posiciones
    data = notificacionesFiltradas.concat(data);
    ...
    for (let card of data) {
        ...
        let notificacion = `
            <whatsapp-chat-notification
                userPhone="${card.telefono_whatsapp}"
                codigoAdmin="wairbotrepair1_CHAT_"
            ></whatsapp-chat-notification>
        `;

        let info1 = card.nombre + "&nbsp;" + notificacion;
...
async function openCliente(id) {
    ...
    let gridMaquinas = await getMaquinasCliente(idOriginal);

    let chat = `
        <div style="width: 100%; height: 500px;">
            <whatsapp-chat-user
                userName="${data.nombre}"
                userPhone="${data.telefono_whatsapp}"
    ...
    let contenido = form + chat + gridMaquinas;
...
//obtener los maquinas de un cliente y mostrarlos en una grid
async function getMaquinasCliente(idCliente) {
    //obtener los datos de los maquinas
    let clase = 'Maquinas';
    ...
    //obtener las cards con los datos
    let cards = cardsMaquinas(data);

    let titulo = "Maquinas";
    let funcionCrear = "crearStock";
    let funcionAvanzar = "avanzarMaquinas";
    let funcionRetroceder = "retrocedMaquinas";
...
function cardsMaquinas(data) {
    let cards = "";
    for (let card of data) {
        let funcionApertura = "openVendido";

        // Verificar si card.imagen es null o undefined y asignar un valor predeterminado
        let imagen = card.imagen ? card.imagen : "imgs/image.png";
        let url = window.location.href;

        if (!imagen.includes("public/")) {
            imagen = "imgs/image.png";
        } else {
            imagen = url + "api/" + imagen;
        }
