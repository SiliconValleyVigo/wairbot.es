...
class Partes{
    static public function create($request){
        ...
        if($tipoParte === 'INSTALACION'){
            //comprobar si el id_cliente existe en $data
            if(array_key_exists('id_cliente', $data) && $data['id_cliente']){
                ...
                if($emailCliente){
                    //enviar email y notificación al cliente
                    $motor = $data;
                    Notificaciones::alCrearParteInstalacion($id_cliente, $emailCliente, $motor);
                }
            }
        ...
        if($tipoParte === 'REPARACION'){
            if(array_key_exists('id_cliente', $data) && $data['id_cliente']){
                ...
                if($emailCliente){
                    //enviar email y notificación al cliente
                    $motor = $data;
                    Notificaciones::alCrearParteReparacion($id_cliente, $emailCliente, $motor);
                }
            }
        ...
        if($tipoParte === 'REVISION'){
            if(array_key_exists('id_cliente', $data) && $data['id_cliente']){
                ...
                if($emailCliente){
                    //enviar email y notificación al cliente
                    $motor = $data;
                    Notificaciones::alCrearParteRevision($id_cliente, $emailCliente, $motor);
                }
            }
        ...
        if(isset($data['estado']) && $data['estado'] === 'CERRADO'){
            ...
            if($tipo === 'INSTALACION'){
                ...
                if($id_cliente){
                    $sql = "SELECT email FROM usuarios WHERE id = $id_cliente";
                    $result = $conn->query($sql);
                    $emailCliente = $result->fetch_assoc()['email'];

                    if($emailCliente){
                        //ENVIAR EMAIL Y NOTIFICACIÓN AL CLIENTE
                        Notificaciones::alCerrarParteInstalacion($id_cliente, $emailCliente, $motor);
                    }
                }
            ...
            if ($tipo === 'REVISION') {
                ...
                if ($id_cliente) {
                    ...
                    if ($emailCliente) {
                        //ENVIAR EMAIL Y NOTIFICACIÓN AL CLIENTE
                        Notificaciones::alCerrarParteRevision($id_cliente, $emailCliente, $motor);
                    }
                }
            ...
            if ($tipo === 'REPARACION') {
                ...
                if ($id_cliente) {
                    ...
                    if ($emailCliente) {
                        //ENVIAR EMAIL Y NOTIFICACIÓN AL CLIENTE
                        Notificaciones::alCerrarParteReparacion($id_cliente, $emailCliente, $motor);
                    }
                }
            }
        }
    ...
    static public function getPartesByTecnicoId($user_id = false){
        $conn = $_SESSION['db_connection'];

        //si hay user_id y user_id no es un array
        if($user_id && !is_array($user_id)){
            $id_tecnico = intval($user_id);
        }else{
