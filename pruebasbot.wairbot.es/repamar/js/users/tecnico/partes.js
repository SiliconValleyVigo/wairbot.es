...
function cardsPartesTecnico(data) {
    ...
    for (let card of data) {
        ...
        let info3 = "Motor: " + card.motor.marca + " " + card.motor.modelo;
...
async function openParteTecnico(id) {
    ...
    let cliente_id = motor[0].id_cliente;
    ...
    //si status es error poner datos de cliente vacios
    if(cliente.status === "error"){
        cliente = {
            data: [{
                nombre: "Sin cliente Asociado",
                direccion: "",
                email: "",
                telefono_contacto: "",
                telefono_whatsapp: ""
    ...
    let fichaMotor = `
        <div class="ficha">
            <div class="fichaTitulo">Motor</div>
            <div class="fichaContenido">
                <div class="fichaCampo"><strong>Marca:&nbsp;</strong> ${motor.marca || ''}</div>
                <div class="fichaCampo"><strong>Modelo:&nbsp;</strong> ${motor.modelo || ''}</div>
                <div class="fichaCampo"><strong>Codigo:&nbsp;</strong> ${motor.codigo || ''}</div>
                <div class="fichaCampo"><strong>Fecha de instalación:&nbsp;</strong> ${motor.fecha_instalacion || ''}</div>
                <div class="fichaCampo"><strong>Observaciones:&nbsp;</strong> ${motor.observaciones || ''}</div>
            </div>
        </div>        
    `;
