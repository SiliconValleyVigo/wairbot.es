class Pagina {
    constructor(config) {
        ...
        this.datosExtraOpen = config.datosExtraOpen || [];
    ...
    async open(id) {
        ...
        //comprobar si datoExtraOpen tiene datos y no es un array vacío
        if (this.datosExtraOpen.length > 0) {
            let campoOculto = document.querySelector(`.card_info_oculto_${id}`);
            if(campoOculto !== null && campoOculto !== undefined){
                let arrayCampoOculto = campoOculto.innerHTML.split('&amp;');
                
                let datosExtraCampoOculto = {};
                for (let campo of arrayCampoOculto) {
                    let arrayCampo = campo.split('::');
                    datosExtraCampoOculto[arrayCampo[0]] = arrayCampo[1];
                }
            
                for (let datoExtra of this.datosExtraOpen) {
                    datos[datoExtra] = datosExtraCampoOculto[datoExtra];
                }
            }
        }
