...
async function crearCamposDeFormulario(campos, data = null, botonEliminar = null){
    ...
    for(let campo of campos){
        ...
        if(tipo === "text"){
            ...
            //comprobar si dentro de campo hay un onchange
            let onchange = "";
            if(campo.hasOwnProperty("onchange")){
                onchange = campo.onchange;
            }

            let input = getSelect(id, label, options, value, onchange);
            camposResult.push(input);
...
function getSelect(id, label, options, value, onchange = ""){
    if(onchange !== ""){
        onchange = `onchange="${onchange}(this.id, this.value)"`;
    }
