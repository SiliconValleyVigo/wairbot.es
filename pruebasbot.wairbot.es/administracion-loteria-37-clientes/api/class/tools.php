    ...
    static public function read($conn, $nPagina, $tabla, $filtro, $datosRelacionados, $search = null, $orderBy = null, $camposRelacionados = []) {
        ...
        if (!empty($camposRelacionados)) {
            foreach ($camposRelacionados as $relacion) {
                ...
                $alias = $idCondicional . "_" . $columna;
                $sql .= ", $alias.$columna AS " . $idCondicional . "_" . $columna;
            }
        }
        ...
        if (!empty($datosRelacionados)) {
            foreach ($datosRelacionados as $nombreColumna => $tablaExtra) {
                ...
                $countSql .= " LEFT JOIN $tablaExtra AS $alias ON $tabla.$nombreColumna = $alias.id";
            }
        }
        ...
        if (!empty($camposRelacionados)) {
            foreach ($camposRelacionados as $relacion) {
                list($tablaExtra, $columna, $idCondicional) = $relacion;
                $alias = $idCondicional . "_" . $columna;
                $sql .= " LEFT JOIN $tablaExtra AS $alias ON $tabla.$idCondicional = $alias.id";
                $countSql .= " LEFT JOIN $tablaExtra AS $alias ON $tabla.$idCondicional = $alias.id";
            }
        }
        ...
        foreach ($dataFormatPending as &$d) {
            ...
            foreach ($camposRelacionados as $relacion) {
                list($tablaExtra, $columna, $idCondicional) = $relacion;
                $alias = $tablaExtra . "_" . $columna;
                $keyCheck = $idCondicional . "_" . $columna;
                if (!isset($d[$keyCheck])) {
                    $d[$keyCheck] = "";
                }
            }
