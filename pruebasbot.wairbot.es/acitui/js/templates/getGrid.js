...
*/
async function getDatosFormulario(campos, camposExtra = null) {
    let datos = {};
    let promesas = [];

    for (let campo of campos) {
        console.log(campo); // { "tipo": "date", "id": "fecha_factura", "label": "Fecha Factura" }
        let id = campo.id;
        console.log(id); // fecha_factura
        console.log(document.getElementById(id)); // <input type="date" id="fecha_factura" value="2024-09-09">
        ...
        // Si el input es file y no está vacío, obtener el archivo en base64 y guardarlo en datos como file_upload y el nombre del archivo con su extensión como dato[id]
        if (campo.tipo === "file" && element.files.length > 0) {
        ...
        //si est tipo file y está vacío eliminar del campo de datos
        if(campo.tipo === "file" && element.files.length === 0){
            delete datos[id];
        }
    }
