async function getTableInput(campo, value, tabla, id) {
    ...
    if (tipo === "text") {
        input = getInputTextTable(inputId, value);
    } else if (tipo === "textarea") {
        input = getInputTextAreaTable(inputId, value);
    } else if (tipo === "file") {
        input = getInputFileTable(value);
    } else if (tipo === "select") {
        input = getSelectTable(inputId, options, value);
...
async function getDataListApiTable(id, columna, value, apiClass, apiFunction){
    //comprobar si la columna ya se encuentra en el array de dataListColeccion
    let options = {};
    if (!dataListColeccion.includes(columna)) {
        ...
        ultimosDatosDataListCompleto = options;
    ...
    let nombre = ultimosDatosDataListCompleto[value];
...
function getInputTextTable(id, value){
    let input = `
        <input
            title="${value}"
...
function getSelectTable(id, options, value){
    let select = `
        <select
            title="${value}"
            style="width: 100%; background-color: transparent; border: none; color: #000;"
